...
//!
//! This policy keeps resident values in FIFO order and records a single
//! visited bit for values used after admission. At eviction time, a moving hand
//! gives visited values one second chance by clearing their bit and continuing
...
const VISITED_WORD_BITS: usize = u64::BITS as usize;
const VISITED_WORDS: usize = PAGE_LEN.div_ceil(VISITED_WORD_BITS);
...
/// SIEVE eviction policy.
///
/// Newly inserted values enter at the front of a FIFO queue. Uses set a
/// page-indexed atomic visited bit; they do not move the value in the queue.
pub struct Sieve {
...
impl EvictionPolicy for Sieve {
    ...
    fn record_insert(&self, id: Id) {
        if let Some(capacity) = self.capacity {
            self.ensure_visited_page(id);
            self.state
                .lock()
                .insert(id, capacity.get(), &self.page_states);
        }
    }

    fn record_use(&self, id: Id) {
        if self.capacity.is_some() {
            if let Some(visited) = self.visited_page_if_allocated(id) {
                visited.record_use(slot_offset(id));
            }
        }
    }
    ...
    fn set_tuning(&mut self, capacity: usize) {
        ...
        if let Some(capacity) = self.capacity {
            ...
            {
                if let Some(visited) = visited_page_if_allocated(page_states, id) {
                    visited.clear(slot_offset(id));
                }
            }
            *state = State::default();
        }
...
impl Sieve {
    fn ensure_visited_page(&self, id: Id) {
        visited_page(&self.page_states, id);
    }

    fn visited_page_if_allocated(&self, id: Id) -> Option<&PageBits> {
        visited_page_if_allocated(&self.page_states, id)
    }
}
...
impl State {
    fn insert(&mut self, id: Id, capacity: usize, page_states: &PageStates) {
        ...
        self.schedule_evictions(capacity, page_states);
    }
    ...
    fn select_victim(&mut self, page_states: &PageStates) -> Option<Id> {
        loop {
            ...
            let visited = visited_page(page_states, id);
            let slot = slot_offset(id);

            if visited.is_visited(slot) {
                visited.clear(slot);
...
impl PageSlot {
    ...
    fn get_or_alloc(&self) -> &PageBits {
        if let Some(page) = self.get() {
            return page;
        }

        let new_page = Box::into_raw(Box::new(PageBits::default()));
...
#[derive(Default)]
struct PageBits {
    words: [AtomicU64; VISITED_WORDS],
}

impl PageBits {
    fn record_use(&self, slot: usize) {
...
fn visited_page(page_states: &PageStates, id: Id) -> &PageBits {
    page_states
        .get_or_alloc(page_state_index(id))
        .get_or_alloc()
}

fn visited_page_if_allocated(page_states: &PageStates, id: Id) -> Option<&PageBits> {
    page_states
...
fn word_and_mask(slot: usize) -> (usize, u64) {
    (slot / VISITED_WORD_BITS, 1 << (slot % VISITED_WORD_BITS))
}
